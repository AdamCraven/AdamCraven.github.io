...
The view models are accessed via the view controllers and view templates to retrieve and set data. The simple objects that are on the scope, are completely replaced by view models. State is no longer stored in the view controllers, it is represented in the view models.
...
Validation should be centered in the model and in advanced use cases - such as multi-field form validation or an update of one model affecting another - a validation controller can be created which knows about every model it needs to validate.
...
Usually a higher level controller will link one or more module APIs together, or create an adapter between them if their inputs and outputs vary.
